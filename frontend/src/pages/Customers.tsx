...
const Customers = () => {
    ...
    const columns = [
        { header: 'Customer Name', accessor: 'name', render: (v: string) => <span className="font-bold text-gray-900">{v}</span> },
        { header: 'Email', accessor: 'email' },
        { header: 'Phone', accessor: 'phone' },
        { header: 'Balance Receivable', accessor: 'balance', render: (v: number) => (
            <span className={`font-bold ${v > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                {format(v)}
            </span>
        )},
        { header: 'Total Sales', accessor: 'totalSales', render: (v: number) => format(v) }
    ];
                        ...
                        <form onSubmit={handleAddCustomer} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-1.5">
                                    ...
                                    <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-gray-50 border border-gray-100 text-gray-900 rounded-2xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 p-3.5 outline-none transition-all" placeholder="John Doe" />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-1">Email Address</label>
                                    <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-gray-50 border border-gray-100 text-gray-900 rounded-2xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 p-3.5 outline-none transition-all" placeholder="john@example.com" />
                                </div>
                            ...
                            <div className="pt-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-1 mb-2 block">Customer Presence / Location</label>
                                <LocationSelector 
